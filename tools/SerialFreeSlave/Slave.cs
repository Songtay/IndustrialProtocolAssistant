...
internal interface ISerialSlaveEngine
{
    /// <summary>引擎说明（寄存器布局/规则条数），启动时打印。</summary>
    string Describe();

    /// <summary>周期刷新模拟数据（由 300ms 定时器调用）。</summary>
    void SimulateTick();
...
//   主站侧 SerialFree 地址用"类 Modbus RTU"请求帧即可直接读写：
...
// =====================================================================================
internal sealed class RtuSlave : ISerialSlaveEngine
{
    public const int RegTemp = 0;     // 0..1   Float  温度（随机游走 20~80°C）
    public const int RegCount = 2;    // 2      UInt16 计数（每 300ms +1，回绕）
...
// =====================================================================================
internal sealed class RuleSlave : ISerialSlaveEngine
{
